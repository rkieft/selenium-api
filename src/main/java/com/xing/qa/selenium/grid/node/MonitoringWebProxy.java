...
public class MonitoringWebProxy extends DefaultRemoteProxy {
    ...
    private static final String URL = format("http://%s:%s", envOr("IFXDB_HOST", "localhost"),
            envOr("IFXDB_PORT", "8086"));
    private static final InfluxDB INFLUX_DB = InfluxDBFactory.connect(URL, envOr("IFXDB_USER", "root"),
    ...
    public MonitoringWebProxy(RegistrationRequest request, GridRegistry registry) {
        ...
        nodeReporter = EXECUTOR.scheduleAtFixedRate(new NodeReporter(remoteHostName, INFLUX_DB, DATABASE, this), 0, 5,
                TimeUnit.SECONDS);
    }
    ...
    public void afterCommand(TestSession session, HttpServletRequest request, HttpServletResponse response) {
        ...
        EXECUTOR.execute(
                new CommandReporter(remoteHostName, INFLUX_DB, DATABASE, session, req, response, ReportType.result));
    ...
    public void beforeCommand(TestSession session, HttpServletRequest request, HttpServletResponse response) {
        ContentSnoopingRequest req = new ContentSnoopingRequest(request);
        EXECUTOR.execute(
                new CommandReporter(remoteHostName, INFLUX_DB, DATABASE, session, req, response, ReportType.command));
